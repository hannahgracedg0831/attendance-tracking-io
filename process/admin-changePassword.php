...
if (count($_POST) > 0) 
{
    $result = mysqli_query($con,"SELECT * From tbl_user WHERE user_id='".$_SESSION["admin_id"]."'");
    ...
    if(password_verify($oldPass,$row['password'])) 
    {
        if($newPass == $renewPass){
            mysqli_query($con,"UPDATE tbl_user Set password='" . password_hash($newPass,PASSWORD_DEFAULT) . "' WHERE user_id='" . $_SESSION["admin_id"] . "'");
