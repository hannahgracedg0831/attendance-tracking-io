...
if(isset($_POST['date'])){ 
	$date=$_POST['date'];
    $delimiter = ",";
    $filename = "staff-data-report_" . date('Y-m-d') . ".csv"; 
     
    // Create a file pointer 
    $f = fopen('php://memory', 'w'); 
     
    // Set column headers 
    $fields = array('Staff ID', 'Staff Name', 'Time In', 'Guard Name', 'Time Out', 'Guard Name', 'Date'); 
			...
			foreach ($value as $k => $v) {
					if($v['date']==$date){
						...
						if($v['type']=='staff'){
                            ...
                            $lineData = array($fetch['staff_id'], $name,$object->TimeIn, $guardIn, $object->TimeOut,  $guardOut, $object->Date); 
